...
- **计算柜**：U 槽层叠 + 6 条刀片带 + 侧边 STATUS LED 光柱
- **交换柜**：密集端口矩阵 + LPO 光模块槽
- **视觉识别**：深色高柜列 + 端口密集交换柜穿插 + 侧边发光 LED 柱

---

## 状态下钻高亮（v2.1）

`entity-builders.js` 通过读取 `C._ACTIVE` 标志实现"下钻高亮"：

- 当外部状态非默认/非离线时（`C._ACTIVE = true`），图元的"下一层级"子元素
  （刀片带、NPU/CPU、Die 功能块、AIC Cube 阵列、HBM 层、端口笼等）
  会用 `mix(base, STATUS, 0.68)` 将状态色**混入**基础灰，而非整体平替。
- 灰阶渐变仍透出（"色相清洗"而非"纯色覆盖"），便于一屏扫视众多节点。
- 默认状态下 `C._ACTIVE` 为假，图元保持中性灰工业外观，向后兼容旧展示库。

`硬件图元库.dc.html`（合并库）在 `_applyStatus` 中设置 `C._ACTIVE` 并重建场景来驱动该效果。
